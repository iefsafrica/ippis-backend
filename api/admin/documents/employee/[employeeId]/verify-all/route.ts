...
export async function POST(request: Request, { params }: { params: { employeeId: string } }) {
  try {
    ...
    await db.execute(updateRegQuery, [
      employeeId,
      comment ? `All documents verified: ${comment}` : "All documents verified",
    ])
    ...
    await db.execute(logActivityQuery, [
      ...
      comment ? `All documents verified with comment: ${comment}` : "All documents verified",
      session.user.email,
    ])
    ...
    return NextResponse.json(
      {
        error: "Failed to verify all documents",
